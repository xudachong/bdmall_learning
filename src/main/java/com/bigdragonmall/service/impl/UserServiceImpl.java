 ...
 * @Description:
 * @Date: Created in 9:58 2017/12/20
 * @
 */
...
public class UserServiceImpl implements IUserService {
	...
	public ServerResponse<User> login(String userName, String password) {
		...
		System.out.println("原密码：" + password + "md5Password" + md5Password);
	...
	//当输完用户名或邮箱之后，点击下一个输入框时，进行一个实时用户名或邮箱是否存在的反馈
	@Override
	public ServerResponse<String> checkValid(String str, String type){
		//isNotBlank当参数是空时返回false，isNotEmpty当参数是空时返回true
	...
	public ServerResponse selectQuestion(String username){
		...
		if(validResponse.isSuccess()){
			//用户不存在
			return ServerResponse.createByErrorMessage("用户不存在，请您先注册或检查登陆信息");
		}
